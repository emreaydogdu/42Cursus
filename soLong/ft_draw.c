...
void	draw_end(t_map *m)
{
    ...
    while (y < m->height - 1)
    {
        ...
        while (x < m->width - 1)
        {
            if (m->map[y][x] == 'E')
            {
                m->end = mlx_texture_to_image(m->window, mlx_load_png("./src/end1.png"));
                mlx_resize_image(m->end, m->end->width * 2, m->end->height * 2);
                mlx_image_to_window(m->window, m->end, (x+1) * 32, (y+1) * 32);
                m->end2 = mlx_texture_to_image(m->window, mlx_load_png("./src/end2.png"));
                mlx_resize_image(m->end2, m->end2->width * 2, m->end2->height * 2);
                mlx_image_to_window(m->window, m->end2, (x+1) * 32, (y+1) * 32);
...
void    draw_collect(t_map *m)
{
    ...
    y = 1;
    i = 0;
    m->collections = malloc(sizeof(t_col*) * m->colcount);
    while (y < m->height - 1)
    {
        x = 1;
        while (x < m->width - 1)
        {
            if (m->map[y][x] == 'C'){
                ...
                //draw_place(m, (x+1) * 32, (y+1) * 32, "./src/collect.png");
            }
            x++;
        }
        y++;
